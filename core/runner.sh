...
# ==========================================================
exec 200>"/tmp/ipguard_runner.lock"
if ! flock -n 200; then
...
log() {
    ...
    local core_msg=$(printf "[%-5s] [%-7s] [%s] %s" "$level" "$module" "$REGION_CODE" "$msg")
...
if [ -t 1 ]; then
    ...
    log "SYSTEM" "INFO " "Scheduler woke in background; random anti-stampede jitter: ${JITTER_TIME}s..."
    sleep $JITTER_TIME
fi
